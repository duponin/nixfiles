...
3. setup disk
   ...
   5. =nixos-generate-config --root /mnt= and save =hardware-configuration.nix= in this repo
   6. =nix-env -iA nixos.git=
   7. =git clone https://git.locahlo.st/duponin/nixfiles /mnt/etc/nixfiles=
   8. =nixos-install --no-root-passwd -I nixos-config=/mnt/etc/nixfiles/configuration/hosts/<HOSTNAME>/configuration.nix=
   9. reboot
   10. enjoy
