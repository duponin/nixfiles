...

{
  services.promtail = {
    ...
    configuration = {
      ...
      clients = [{
        url = "http://pluto.locahlost.net:3100/loki/api/v1/push";
      }];
      # heavily inspired of https://github.com/Mic92/dotfiles/blob/main/nixos/modules/promtail.nix
      scrape_configs = [
        {
          job_name = "journal";
          journal = {
          ...
          pipeline_stages = [
            ...
            }
            {
              template = {
                ...
                template =
                  "{{if .coredump_exe}}{{.coredump_exe}} core dumped (user: {{.coredump_uid}}/{{.coredump_gid}}, command: {{.coredump_cmdline}}){{else}}{{.msg}}{{end}}";
              };
            }
